...
// pData: 字模数据指针（32字节）
// color: 字体颜色
void LCD_ShowChinese16(u16 x, u16 y, const unsigned char *pData, u16 color)
{
    u16 i, j;
    u8 temp;
    ...
    for (i = 0; i < 16; i++) {
        for (j = 0; j < 2; j++) {
            ...
            for (u8 k = 0; k < 8; k++) {
                if (temp & (0x80 >> k)) {
                    LCD_Fast_DrawPoint(x + j * 8 + k, y + i, color);
                }
            }
        }
    }
...
// ========== 显示中文字符串（16x16） ==========
// 支持：温度湿度阈值主界面设置当前返回切换加减
void LCD_ShowChineseString16(u16 x, u16 y, const char *str, u16 color)
{
    u16 x0 = x;
    ...
    while (*str) {
        if ((unsigned char)*str >= 0x80) {  // 中文字符
            ...
            if (strncmp(str, "温", 2) == 0) pData = chinese_16_temp;
            else if (strncmp(str, "湿", 2) == 0) pData = chinese_16_humidity;
            else if (strncmp(str, "度", 2) == 0) pData = chinese_16_degree;
            else if (strncmp(str, "阈", 2) == 0) pData = chinese_16_threshold;
            else if (strncmp(str, "值", 2) == 0) pData = chinese_16_value;
            else if (strncmp(str, "主", 2) == 0) pData = chinese_16_main;
            else if (strncmp(str, "界", 2) == 0) pData = chinese_16_boundary;
            else if (strncmp(str, "面", 2) == 0) pData = chinese_16_face;
            else if (strncmp(str, "设", 2) == 0) pData = chinese_16_set;
            else if (strncmp(str, "置", 2) == 0) pData = chinese_16_place;
            else if (strncmp(str, "当", 2) == 0) pData = chinese_16_current;
            else if (strncmp(str, "前", 2) == 0) pData = chinese_16_front;
            else if (strncmp(str, "返", 2) == 0) pData = chinese_16_return;
            else if (strncmp(str, "回", 2) == 0) pData = chinese_16_back;
            else if (strncmp(str, "切", 2) == 0) pData = chinese_16_cut;
            else if (strncmp(str, "换", 2) == 0) pData = chinese_16_change;
            else if (strncmp(str, "加", 2) == 0) pData = chinese_16_add;
            else if (strncmp(str, "减", 2) == 0) pData = chinese_16_sub;
            else if (strncmp(str, "监", 2) == 0) pData = chinese_16_monitor;
            else if (strncmp(str, "控", 2) == 0) pData = chinese_16_control;
            
            if (pData != NULL) {
                LCD_ShowChinese16(x0, y, pData, color);
            }
            x0 += 16;
            str += 2;
        } else {
            // 英文字符（使用16x16字体）
            LCD_ShowChar(x0, y, *str, 16, 0);
            x0 += 8;
            str++;
        }
    }
