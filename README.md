...
│   │                          上行：采集 + 上报                            │   │
│   │                                                                       │   │
│   │   Sensor_Task              g_sensorData              Upload_Task      │   │
│   │   ┌────────────┐          ┌────────────┐          ┌────────────┐     │   │
│   │   │ DHT11 采集 │───写────?│ Temp / Hum │───读────?│ JSON 上报  │     │   │
│   │   │ 每 5 秒    │          │ Led/Alarm  │          │ 每 5 秒    │     │   │
│   │   └────────────┘          └────────────┘          └────────────┘     │   │
│   └──────────────────────────────────────────────────────────────────────┘   │
│                                                                               │
│   ┌──────────────────────────────────────────────────────────────────────┐   │
│   │                          下行：接收 + 控制                            │   │
│   │                                                                       │   │
│   │   ParserTask            OneNET_Process              g_sensorData      │   │
│   │   ┌────────────┐        Command                   ┌────────────┐     │   │
│   │   │ MQTT 解析  │───调──?│ 解析 JSON  │───写────?│ 更新状态   │     │   │
│   │   │ PUBLISH    │        │ 控制 LED   │          │ Led/Alarm  │     │   │
│   │   └────────────┘        │ 控制蜂鸣器 │          └────────────┘     │   │
│   │                         │ 回复云端   │                              │   │
│   │                         └────────────┘                              │   │
│   └──────────────────────────────────────────────────────────────────────┘   │
